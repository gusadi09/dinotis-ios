    ...
    var body: some View {
        ...
        GeometryReader { geo in
            ZStack(alignment: .center) {
                                    ...
                                    LazyVStack(spacing: 15) {
                                        if viewModel.rateCardList.isEmpty {
                                            ...
                                            ForEach(viewModel.rateCardList.unique(), id: \.id) { item in
                                                ...
                                                .padding(.horizontal)
                                                .onAppear {
                                                    if (viewModel.rateCardList.unique().last?.id).orEmpty() == item.id.orEmpty() {
                                                        viewModel.query.skip = viewModel.query.take
                                                        viewModel.query.take += 15
                                                        viewModel.getRateCardList(by: (viewModel.talentData?.id).orEmpty())
                                                    }
                                                }
                                            }
                                        }
                ...
                LoadingView(isAnimating: $viewModel.isLoading, isFullBackground: true)
