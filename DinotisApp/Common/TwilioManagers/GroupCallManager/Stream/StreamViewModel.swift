...
class StreamViewModel: ObservableObject {
    ...
    func configure(
        ...
        streamManager.speakerInvitePublisher
        ...
        //FIXME: - Change to firebase
        streamManager.speakerLockAudio
            .sink { [weak self] value in
                self?.isAudioLocked = value
                
                if !value && StateObservable.shared.twilioRole != "host" && StateObservable.shared.twilioRole != "viewer" {
                    self?.alertIdentifier = .unlockedMuteByHost
                } else if value && StateObservable.shared.twilioRole != "host" && StateObservable.shared.twilioRole != "viewer" {
                    self?.alertIdentifier = .mutedByHost
                }
            }
            .store(in: &subscriptions)
        ...
        //FIXME: - Change to firebase
        streamManager.spotlightedIdentity
            .sink { value in
