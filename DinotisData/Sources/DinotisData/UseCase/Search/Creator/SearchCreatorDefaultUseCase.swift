...
public struct SearchCreatorDefaultUseCase: SearchCreatorUseCase {
    ...
    public func execute(query: SearchQueryParam) async -> Result<SearchedResponse, Error> {
        do {
            let result = try await repository.provideGetSearchCreator(query: query)
