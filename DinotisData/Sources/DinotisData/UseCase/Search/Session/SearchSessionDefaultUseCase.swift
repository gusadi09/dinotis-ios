...
public struct SearchSessionDefaultUseCase: SearchSessionUseCase {
    ...
    public func execute(query: SearchQueryParam) async -> Result<SearchResponse, Error> {
        do {
            let result = try await repository.provideGetSearchData(query: query)
