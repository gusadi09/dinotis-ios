...
public struct GetBundlingDetailDefaultUseCase: GetBundlingDetailUseCase {
    ...
    public func execute(by bundleId: String) async -> Result<DetailBundlingResponse, Error> {
        do {
            let data = try await bundlingRepository.provideGetDetailBundling(by: bundleId)
            ...
            if error.statusCode.orZero() == 401 {
                ...
                case .success(let token):
                    ...
                    let data = await execute(by: bundleId)
