...
public struct GetBundlingListDefaultUseCase: GetBundlingListUseCase {
    ...
    public func execute(query: BundlingListFilter) async -> Result<BundlingListResponse, Error> {
        do {
            let data = try await bundlingRepository.provideGetBundlingList(query: query)
