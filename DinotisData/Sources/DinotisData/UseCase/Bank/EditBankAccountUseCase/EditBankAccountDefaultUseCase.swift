...
public struct EditBankAccountDefaultUseCase: EditBankAccountUseCase {
    ...
    public func execute(for id: Int, with query: AddBankAccountRequest) async -> Result<BankAccountData, Error> {
        do {
            let data = try await repository.provideEditBankAccount(by: id, contain: query)
            ...
            if error.statusCode.orZero() == 401 {
                ...
                case .success(let token):
                    ...
                    let data = await execute(for: id, with: query)
                    return data
