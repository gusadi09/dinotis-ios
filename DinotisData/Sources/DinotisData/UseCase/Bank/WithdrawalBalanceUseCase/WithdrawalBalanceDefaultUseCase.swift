...
public struct WithdrawalBalanceDefaultUseCase: WithdrawalBalanceUseCase {
    ...
    public func execute(with request: WithdrawalRequest) async -> Result<WithdrawDetailResponse, Error> {
        do {
            let data = try await repository.provideWithdrawBalance(by: request)
