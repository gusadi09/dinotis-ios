...
public struct EditUserDefaultUseCase: EditUserUseCase {
    ...
    public func execute(with body: EditUserRequest) async -> Result<UserResponse, Error> {
        do {
            let data = try await repository.provideUpdateUser(with: body)
