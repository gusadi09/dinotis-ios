...
public struct CurrentBalanceDefaultUseCase: CurrentBalanceUseCase {
    ...
    public func execute() async -> Result<String, Error> {
        do {
            let data = try await repository.provideUserCurrentBalance()
            
            return .success(data.current.orEmpty())
