...
public struct SetCreatorAvailabilityDefaultUseCase: SetCreatorAvailabilityUseCase {
    ...
    public func execute(with body: CreatorAvailabilityRequest) async -> Result<String, Error> {
        do {
            let data = try await repository.provideCreatorAvailability(body: body)
