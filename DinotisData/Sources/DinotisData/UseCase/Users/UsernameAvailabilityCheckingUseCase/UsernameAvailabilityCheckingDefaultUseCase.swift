...
public struct UsernameAvailabilityCheckingDefaultUseCase: UsernameAvailabilityCheckingUseCase {
    ...
    public func execute(with username: UsernameAvailabilityRequest) async -> Result<SuccessResponse, Error> {
        do {
            let data = try await repository.provideUsernameAvailability(with: username)
            ...
            if error.statusCode.orZero() == 401 {
                ...
                case .success(let token):
                    ...
                    let data = await execute(with: username)
                    return data
