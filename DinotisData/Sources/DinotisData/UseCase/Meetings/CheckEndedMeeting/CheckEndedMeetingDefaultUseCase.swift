...
public struct CheckEndedMeetingDefaultUseCase: CheckEndedMeetingUseCase {
    ...
    public func execute(for meetingId: String) async -> Result<CheckingEndMeetingResponse, Error> {
        do {
            let data = try await repository.providePostCheckMeetingEnd(meetingId: meetingId)
