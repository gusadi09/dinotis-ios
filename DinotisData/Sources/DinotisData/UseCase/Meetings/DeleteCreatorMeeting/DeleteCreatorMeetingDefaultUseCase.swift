...
public struct DeleteCreatorMeetingDefaultUseCase: DeleteCreatorMeetingUseCase {
    ...
    public func execute(for meetingId: String) async -> Result<SuccessResponse, Error> {
        do {
            let data = try await repository.provideDeleteMeeting(meetingId: meetingId)
