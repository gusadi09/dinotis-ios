...
public struct GetClosestSessionDefaultUseCase: GetClosestSessionUseCase {
    ...
    public func execute() async -> Result<ClosestMeetingResponse, Error> {
        do {
            let data = try await repository.provideGetClosestSession()
