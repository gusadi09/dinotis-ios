...
public struct GetRecordingsDefaultUseCase: GetRecordingsUseCase {
    ...
    public func execute(for meetingId: String) async -> Result<RecordingResponse, Error> {
        do {
            let data = try await repository.provideGetRecordings(for: meetingId)
