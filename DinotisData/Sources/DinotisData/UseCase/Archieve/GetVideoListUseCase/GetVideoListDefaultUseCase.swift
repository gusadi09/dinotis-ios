//
//  GetVideoListDefaultUseCase.swift
//
//
...
public struct GetVideoListDefaultUseCase: GetVideoListUseCase {
    ...
    public func execute(with param: VideoListRequest) async -> Result<MineVideoResponse, Error> {
        do {
            let data = try await repository.provideGetVideoList(with: param)
            ...
            if error.statusCode.orZero() == 401 {
                ...
                case .success(let token):
                    ...
                    let data = await execute(with: param)
                    return data
