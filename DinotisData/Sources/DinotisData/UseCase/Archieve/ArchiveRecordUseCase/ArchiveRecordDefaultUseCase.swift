...
public struct ArchiveRecordDefaultUseCase: ArchiveRecordUseCase {
    ...
    public func execute(with meetingId: String) async -> Result<String, Error> {
        do {
            let data = try await repository.providePostArchive(meetingId: meetingId)
            ...
            if error.statusCode.orZero() == 401 {
                ...
                case .success(let token):
                    ...
                    let data = await execute(with: meetingId)
                    return data
