...
public struct PostCommentDefaultUseCase: PostCommentUseCase {
    ...
    public func execute(for id: String, comment: String) async -> Result<String, Error> {
        do {
            let data = try await repository.providePostComment(videoId: id, comment: comment)
            ...
            if error.statusCode.orZero() == 401 {
                ...
                case .success(let token):
                    ...
                    let data = await execute(for: id, comment: comment)
                    return data
