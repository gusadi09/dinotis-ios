...
public struct EditVideoItemDefaultUseCase: EditVideoItemUseCase {
    ...
    public func execute(for id: String, with body: VideosRequest) async -> Result<SuccessResponse, Error> {
        do {
            let data = try await repository.provideEditVideo(on: id, with: body)
            ...
            if error.statusCode.orZero() == 401 {
                ...
                case .success(let token):
                    ...
                    let data = await execute(for: id, with: body)
                    return data
