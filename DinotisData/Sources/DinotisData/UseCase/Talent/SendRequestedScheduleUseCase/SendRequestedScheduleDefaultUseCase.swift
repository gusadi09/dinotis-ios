...
public struct SendRequestedScheduleDefaultUseCase: SendRequestedScheduleUseCase {
    ...
    public func execute(with talentId: String, for body: SendScheduleRequest) async -> Result<RequestScheduleResponse, Error> {
        do {
            let result = try await repository.provideSendRequestSchedule(talentId: talentId, body: body)
            ...
            if error.statusCode.orZero() == 401 {
                ...
                case .success(let token):
                    ...
                    let data = await execute(with: talentId, for: body)
                    return data
