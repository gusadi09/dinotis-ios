...
public struct GetReccomendationTalentDefaultUseCase: GetReccomendationTalentUseCase {
    ...
    public func execute(query: TalentsRequest) async -> Result<SearchTalentResponse, Error> {
        do {
            let result = try await repository.provideGetRecommendationTalent(with: query)
