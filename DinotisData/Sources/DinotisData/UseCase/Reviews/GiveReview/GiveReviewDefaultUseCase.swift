...
public struct GiveReviewDefaultUseCase: GiveReviewUseCase {
    ...
    public func execute(with body: ReviewRequestBody) async -> Result<ReviewSuccessResponse, Error> {
        do {
            let data = try await repository.provideGiveReview(with: body)
