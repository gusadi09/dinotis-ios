...
public struct ResetPasswordDefaultUseCase: ResetPasswordUseCase {
    ...
    public func execute(with params: ResetPasswordRequest) async -> Result<UserResponse, Error> {
        do {
            let data = try await repository.resetPassword(with: params)
