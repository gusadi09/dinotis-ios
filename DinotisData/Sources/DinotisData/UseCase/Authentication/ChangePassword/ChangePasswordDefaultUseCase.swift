...
public struct ChangePasswordDefaultUseCase: ChangePasswordUseCase {
    ...
    public func execute(with params: ChangePasswordRequest) async -> Result<SuccessResponse, Error> {
        do {
            let data = try await repository.changePassword(with: params)
