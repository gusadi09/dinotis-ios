...
public struct SendPanicReportDefaultUseCase: SendPanicReportUseCase {
    ...
    public init(
        repository: ReportRepository = ReportDefaultRepository(),
    ...
    public func execute(with params: ReportRequest) async -> Result<ReportResponse, Error> {
        do {
            let data = try await repository.provideSendPanicReport(with: params)
