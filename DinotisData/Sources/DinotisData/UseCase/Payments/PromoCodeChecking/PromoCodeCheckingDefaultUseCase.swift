...
public struct PromoCodeCheckingDefaultUseCase: PromoCodeCheckingUseCase {
	...
	public func execute(with request: PromoCodeRequest) async -> Result<PromoCodeResponse, Error> {
		do {
			let data = try await paymentRepository.providePromoCodeChecking(body: request)
