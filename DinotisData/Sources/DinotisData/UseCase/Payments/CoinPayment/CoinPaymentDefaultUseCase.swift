...
public struct CoinPaymentDefaultUseCase: CoinPaymentUseCase {
	...
	public func execute(with request: CoinPaymentRequest) async -> Result<UserBookingData, Error> {
		do {
			let data = try await paymentRepository.provideCoinPayment(body: request)
