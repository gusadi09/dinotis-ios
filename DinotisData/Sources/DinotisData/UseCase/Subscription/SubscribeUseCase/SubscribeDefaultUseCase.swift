...
public struct SubscribeDefaultUseCase: SubscribeUseCase {
    ...
    public func execute(for userId: String, with methodId: Int) async -> Result<SubscriptionResponse, Error> {
        do {
            let data = try await repository.providePostSubscribe(userId: userId, methodId: methodId)
            ...
            if error.statusCode.orZero() == 401 {
                ...
                case .success(let token):
                    ...
                    let data = await execute(for: userId, with: methodId)
                    return data
