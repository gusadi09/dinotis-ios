...
public struct CreateRateCardDefaultUseCase: CreateRateCardUseCase {
	...
	public func execute(with params: CreateRateCardRequest) async -> Result<RateCardResponse, Error> {
		do {
			let data = try await repository.provideCreateRateCard(with: params)
