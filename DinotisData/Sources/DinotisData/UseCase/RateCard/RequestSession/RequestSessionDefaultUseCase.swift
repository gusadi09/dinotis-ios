...
public struct RequestSessionDefaultUseCase: RequestSessionUseCase {
	...
	public func execute(with params: RequestSessionRequest) async -> Result<UserBookingData, Error> {
		do {
			let data = try await repository.provideRequestSession(with: params)
