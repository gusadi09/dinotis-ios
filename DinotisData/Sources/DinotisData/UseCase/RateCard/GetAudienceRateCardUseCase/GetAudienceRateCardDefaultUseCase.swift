...
public struct GetAudienceRateCardDefaultUseCase: GetAudienceRateCardUseCase {
	...
	public func execute(for id: String, with params: RateCardFilterRequest) async -> Result<RateCardListResponse, Error> {
		do {
			let data = try await repository.provideUserGetRateCardList(by: id, query: params)
			...
			if error.statusCode.orZero() == 401 {
				...
				case .success(let token):
					...
					let data = await execute(for: id, with: params)
					return data
