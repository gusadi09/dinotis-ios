...
public struct MeetingRequestDefaultUseCase: MeetingRequestUseCase {
	...
	public func execute(with params: RateCardFilterRequest) async -> Result<MeetingRequestsResponse, Error> {
		do {
			let data = try await repository.provideGetTalentMeetingRequests(by: params)
