...
public struct EditRateCardDefaultUseCase: EditRateCardUseCase {
	...
	public func execute(for id: String, with params: CreateRateCardRequest) async -> Result<RateCardResponse, Error> {
		do {
			let data = try await repository.provideEditRateCard(by: id, contain: params)
