...
public struct GetRateCardListDefaultUseCase: GetRateCardListUseCase {
    ...
    public func execute(with request: HomeContentRequest) async -> Result<DataResponse<RateCardResponse>, Error> {
        do {
            let data = try await repository.provideGetRateCardList(with: request)
