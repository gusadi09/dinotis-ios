...
public struct GetParticipantsDefaultUseCase: GetParticipantsUseCase {
	...
	public func execute(by bookingId: String) async -> Result<ParticipantResponse, Error> {
		do {
			let data = try await bookingsRepository.provideGetParticipant(by: bookingId)
