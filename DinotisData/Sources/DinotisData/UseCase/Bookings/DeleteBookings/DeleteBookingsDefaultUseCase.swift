...
public struct DeleteBookingsDefaultUseCase: DeleteBookingsUseCase {
	...
	public func execute(by bookingId: String) async -> Result<String, Error> {
		do {
			let data = try await bookingsRepository.provideDeleteBooking(by: bookingId)
